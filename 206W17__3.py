...
## SI 206 - W17 - HW3
## COMMENT WITH:
## Your section day/time: Thurs 6-7pm
...
## e.g. 101 Dalmations, inside it, and should return either the LAST correctly matching number-word combo in the string as a tuple, 
...
def parse_counted_words(wordstring):
    regex= r"\d+\s\S+[A-Za-z]"
    matchlist= re.findall(regex, wordstring)
    try:
        lastmatch= matchlist[-1]
        count, word= lastmatch.split()
        matchtuple= (count, word)
    except:
        return None
...
## PART 2: 200 points
filepath= "/Users/mac/SI206P3/computer_paths.txt"
f= open(filepath, 'r')
data= f.read()


## We have provided a text file computer_paths.txt. It's not incredibly long -- you can scan through it, but do NOT hard code your answers! 
## Each line contains 1 filesystem path.
...
python_course_paths= len(Si206py)
## (d) Write Python code to determine how many of these paths describe a Microsoft file (a file that EITHER ends with .docx OR .xlsx, but nothing else counts) 
## where the file name ends in a digit. Save that total in the variable microsoft_files_num.
regex= r"(\d\.docx|\d\.xlsx)"
ms_files= re.findall(regex, data, re.MULTILINE)
microsoft_files_num= len(ms_files)
